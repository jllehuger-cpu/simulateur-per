...
export default function SuccessionPage() {
  ...
  const calculs = useMemo(() => {
    ...
    // Règle Spécifique : Exonération Conjoint / PACS en succession (Art. 796-0 bis CGI)
    if (lienParente === 'conjoint') {
      return { exonere: true, abattement: Infinity, droitsTotaux: 0, detailTranches: [] };
    }
  ...
  if (!baremes || !calculs) return <div className="p-10 text-center font-bold text-slate-400 uppercase tracking-widest">Calcul des droits de succession...</div>;
    ...
    <main className="max-w-6xl mx-auto p-4 md:p-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm h-fit">
          <h2 className="text-lg font-bold mb-6">Paramètres Succession</h2>
          <div className="space-y-6">
            <div>
              ...
              <select 
                ...
                className="w-full p-3 bg-slate-50 border-2 border-slate-100 rounded-xl font-bold outline-none focus:border-slate-900"
              >
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Actif net taxable (€)</label>
              <input 
                type="number" 
                value={patrimoineTaxable} 
                onChange={e => setPatrimoineTaxable(Number(e.target.value))} 
                className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-2xl outline-none focus:border-slate-900" 
              />
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {calculs.exonere ? (
            <div className="bg-emerald-50 border-2 border-emerald-100 p-8 rounded-3xl text-center">
              <p className="text-emerald-700 font-bold">Le conjoint survivant et le partenaire de PACS sont totalement exonérés de droits de succession (Art. 796-0 bis du CGI).</p>
            </div>
          ) : (
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                <h3 className="font-bold text-slate-800">Barème d'imposition</h3>
                <span className="text-[10px] font-bold bg-slate-100 px-3 py-1 rounded-full text-slate-500 uppercase">Part taxable : {Math.round(calculs.assietteTaxable).toLocaleString()} €</span>
              </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {calculs.detailTranches.map((tranche, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-6 py-4 text-center"><span className="px-2 py-1 bg-slate-100 rounded text-[10px] font-black">{tranche.taux}%</span></td>
                      <td className="px-6 py-4 text-right text-sm font-black">{Math.round(tranche.impot).toLocaleString()} €</td>
                    </tr>
                  ))}
                  {calculs.detailTranches.length === 0 && (
                    <tr><td colSpan={4} className="px-6 py-12 text-center text-emerald-500 font-bold uppercase">Aucun droit à payer</td></tr>
                  )}
                </tbody>
              </table>
            </div>
